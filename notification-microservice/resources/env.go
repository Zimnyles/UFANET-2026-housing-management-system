...
type Env struct {
	ServiceName      string        `envconfig:"APP_NAME" default:"notification-service"`
	Host             string        `envconfig:"APP_HOST" default:"0.0.0.0"`
	Port             int           `envconfig:"APP_PORT" default:"50055"`
	...
	PostgresDB       string        `envconfig:"POSTGRES_DB" default:"notifications"`
...
func (e *Env) Addr() string { return fmt.Sprintf("%s:%d", e.Host, e.Port) }
func (e *Env) DSN() string {
	return fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=%s", e.PostgresHost, e.PostgresPort, e.PostgresUser, e.PostgresPassword, e.PostgresDB, e.PostgresSSLMode)
}
