...
func (s *RequestsService) CreateRequest(ctx context.Context, req *domain.CreateRequestRequest) (*domain.MaintenanceRequest, error) {
	r, err := s.repo.Create(ctx, createRequestToModel(req))
	if err != nil {
...
func (s *RequestsService) AddComment(ctx context.Context, req *domain.AddCommentRequest) (*domain.Comment, error) {
	comment, err := s.repo.AddComment(ctx, addCommentToModel(req))
	if err != nil {
...
func (s *RequestsService) publishStatusUpdated(ctx context.Context, r *domain.MaintenanceRequest, _ string) {
	if s.publisher == nil {
		return
	}

	evt := requestStatusUpdatedEvent(r)
...
func (s *RequestsService) publishCommentAdded(ctx context.Context, comment *domain.Comment) {
	...
	evt := requestCommentAddedEvent(comment)
